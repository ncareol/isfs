...
public class HtLookup extends QDialog {

    /**
     * The dialog that provides users with UI interface to select height
     * variables for a new page It provides users with a table to populate the
     * height variables, a search-engine to find desirable variables, a toggle
     * button to select/deselect vars, a check or uncheck a variable, select
     * whole/deselect whole vars in the current table
     * 
     * @param owner
     */
    ...
    private void searchBegin() {
        ...
        for (int i = 0; i < str.length(); i++) {
            char c = str.charAt(i);
            char ch = '.';
            if (!Character.isDigit(c) && !Character.isDefined(ch))
                return;
        }
        if (String.valueOf(str.charAt(strlen-1)).equals('.')) str = str + "0"; // if input=2. then it should be 2.0
        str = str + "m"; //add meter
        
        setCursor(new QCursor(Qt.CursorShape.WaitCursor));
        int len = _vars.size(); _selectedIdxs.clear();
        for (int i = 0; i < len; i++) {
            String vname = _vars.get(i).getName();
            if (vname.length() < str.length()) {
                _tbl.setRowHidden(i, true);                
            } else {
                int idx = vname.indexOf(str);
                if (idx > -1) {
                    _tbl.setRowHidden(i, false);
                    _selectedIdxs.add(new Integer(i));
                } else {
